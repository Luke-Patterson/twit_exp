...
for i in nodes_dict.keys():
    for j in nodes_dict.keys():
        if i != j:
            compar_df.iloc[i,j] = user_overlap(nodes_dict[i],nodes_dict[j])
            compar_df_alt.iloc[i,j] = user_overlap(nodes_dict[i],nodes_dict[j],True,i,j)
        else:
            compar_df.iloc[i,j] = np.nan
            compar_df_alt.iloc[i,j] = np.nan
# look at average overlap
#print(compar_df.mean().mean())
#print(compar_df_alt.mean().mean())

compar_df.to_csv('output/clustering/overlap_matrix.csv')
compar_df_alt.to_csv('output/clustering/overlap_matrix_alt.csv')
...
for n, c_df in enumerate([compar_df, compar_df_alt]):
    ...
    if n == 0:
        df.to_csv('output/clustering/announce_communities.csv')
    else:
        df.to_csv('output/clustering/announce_communities_alt.csv')
